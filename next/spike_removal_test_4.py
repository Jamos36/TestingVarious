...
def remove_spikes_from_envelope(df, x_col, y_col, threshold=0.15):
    """
    Remove spikes using second derivative discontinuity detection.
    
    Identifies spikes as points where the second derivative (rate of change of slope)
    exceeds a threshold, indicating an abrupt change in the curve's acceleration.
    ...
    y_col : str
        Name of y column
    threshold : float
        Second derivative magnitude threshold
    ...
    for i in range(1, len(df) - 1):
        ...
        if dx1 > 0 and dx2 > 0:
            ...
            first_deriv_left = dy1 / dx1
            first_deriv_right = dy2 / dx2
            
            avg_dx = (dx1 + dx2) / 2
            second_deriv = (first_deriv_right - first_deriv_left) / avg_dx
...
def process_with_spike_removal(df, x_col, y_col, thresholds=[0.05, 0.1, 0.15, 0.2], 
                               output_dir=None, plot_output=None):
    """
    Process envelope with multiple second derivative thresholds and generate comparison plots.
    ...
    y_col : str
        Name of y column
    thresholds : list
        List of second derivative magnitude thresholds
    ...
    if plot_output:
        ...
        for idx, (threshold, ax) in enumerate(zip(thresholds, axes)):
            ...
            ax.set_title(f'Second derivative threshold = {threshold:.2f} | Spikes removed: {spike_count}', 
...
if __name__ == '__main__':
    ...
    results = process_with_spike_removal(test_df, 'x', 'y', 
                                        output_dir='spike_removal_test_4',
                                        plot_output='spike_removal_test_4/comparison.png')
    
    print("Spike removal results (Derivative-based):")
