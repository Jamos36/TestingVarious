...
def method1_variable_width_bins(df, x_col, y_col, baseline_bins=70, output_dir=None):
    """
    Variable-width bins based on regional density.
    - Start with baseline uniform bins
    - Count points in each baseline bin
    - Subdivide: Sparse→1x, Medium→3x, Dense→8x
    - Result: more bins in dense regions, fewer in sparse
    ...
    sparse_threshold = mean_count - 0.7 * std_count
    dense_threshold = mean_count + 0.7 * std_count
...
if __name__ == '__main__':
    ...
    result = method1_variable_width_bins(test_df, 'x', 'y', output_dir='test_output')
